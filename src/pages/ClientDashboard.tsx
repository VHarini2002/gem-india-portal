...
import { LogOut, Search, HelpCircle, Plane } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-body">Engine Asset Lifecycle</p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        ...
        >
          <div className="relative h-48 md:h-56">
            ...
            />
            <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.6) 50%, rgba(255,255,255,0.2) 100%)' }} />
            <div className="relative z-10 h-full flex items-center px-8">
              <div>
                ...
                <div className="flex gap-4 mt-4">
                  {[
                    { label: 'Active', count: clientEngines.filter(e => e.status !== 'Completed').length, color: 'bg-primary/10 text-primary' },
                    { label: 'In Transit', count: clientEngines.filter(e => e.status === 'In Transit').length, color: 'bg-warning/15 text-warning' },
                    { label: 'In Storage', count: clientEngines.filter(e => e.status === 'In Storage').length, color: 'bg-success/10 text-success' },
                  ].map(s => (
